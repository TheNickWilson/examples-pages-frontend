...
class LocationControllerSpec extends ControllerSpecBase with MockitoSugar {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new LocationController(frontendAppConfig, messagesApi, FakeDataCacheConnector, mockNavigator, FakeIdentifierAction,
  ...
  "Location Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(LocationPage.toString -> JsString(Location.values.head.toString))
